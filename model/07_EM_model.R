...
fullmodel <- function(GeneByCellExpression, Cellanno=Cellanno, Pseudotime, Design, GeneName, num.knot=3){
  ...
  for (i in seq(1, nrow(Design))){
    ...
    Y_s[[i]] <- GeneByCellExpression[id, row.names(Phi_s[[i]]), drop=F] ##
  }
  names(Y_s) <- names(X_s) <- names(Phi_s) <- row.names(Design)
  
  X <- do.call(rbind, X_s) 
  Y <- t(do.call(cbind,Y_s)) 
  
  phi <- as.matrix(.bdiag(Phi_s))
  X_til <- phi %*% X
  beta <- solve(t(X_til) %*% X_til) %*% t(X_til) %*% Y
  ## initialize tau
  fit <- sapply(names(X_s), function(ss){
    tmp <- t(Phi_s[[ss]])
    phi_x <- t(tmp) %*% X_s[[ss]]
    v <- solve(tmp %*% t(tmp)) %*% tmp %*% t(Y_s[[ss]]) - X_s[[ss]] %*% beta
  })
  tau <- cov(t(fit))
  ## initialize gamma
  gamma <- sapply(names(X_s), function(ss){
    var(t(Y_s[[ss]]) - Phi_s[[ss]] %*% (solve(t(Phi_s[[ss]]) %*% Phi_s[[ss]]) %*% t(Phi_s[[ss]]) %*% t(Y_s[[ss]])))
  })
  ### E step
  ...
  return(list(beta = beta, gamma = gamma, tau = tau, logL = unlist(logLlist)))
}

res <- fullmodel(GeneByCellExpression=expr,
Design=design,
num.knot = 3,
GeneName =  'NCBP3',
Cellanno = data.frame(cell=colnames(expr), sample = sub(':.*','', colnames(expr)), stringsAsFactors = FALSE))

plot(unlist(res$logL))
res$beta[[20]]

{

  ##### check u
  e_s_tmp <- t(GeneByCellExpression[grep(GeneName,row.names(GeneByCellExpression)), Cellanno[,'sample']=='BM1',drop=F])
