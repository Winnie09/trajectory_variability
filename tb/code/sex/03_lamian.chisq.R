...
res <- testpt(expr=paste0(ddir, 'exprpc',pid,'.h5'), cellanno=cellanno, pseudotime=pt, design=design,testvar=ncol(design), test.type = 'Variable', demean = FALSE, overall.only = F, test.method = 'chisq', ncores = 20)
saveRDS(res,file=paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/tb/res/sex_addBatch/pc',pid,'/lamian_chisq.rds'))
