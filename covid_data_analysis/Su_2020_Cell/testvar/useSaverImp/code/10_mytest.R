...
design <- readRDS('covid/Su_2020_Cell/data/design_numeric_Recovered_Deceased.rds')
...
system.time({
  res <- testpt(expr=expr[1:3, ], cellanno=cellanno, pseudotime=pt, design=design, test.type='Variable', ncores = 4, demean = FALSE, test.method = ifelse(m == 'EM_pm', 'permutation', 'chisq'))
})
