...
##### select group
cellid <- cellanno[cellanno[,2] %in% g1,1]
expr=expr[,cellid]
cellanno=cellanno[cellid,]
pseudotime=pt[cellid]
design=design[g1,1,drop=F]
savefn <- paste0(rdir, 'testtime_res_subgroup1.rds')
...
if (grepl('tradeSeq', method)){
  ...
  saveRDS(sce, paste0(rdir, 'testtime_res_subgroup1_sce.rds'))
