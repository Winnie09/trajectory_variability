...
design <- readRDS('covid/Su_2020_Cell/data/design_numeric_HD_Mod.rds')
...
system.time({
  res <- testpt(expr=expr, cellanno=cellanno, pseudotime=pt, design=design, type='Variable', ncores = 16, demean = FALSE)
})
saveRDS(res, paste0(rdir, 'numeric_HD_Mod_res.rds'))
