...
rdir <- './addsignal/data/ndPlainAll/1256/'
if (!file.exists(paste0(rdir, addSignalType,'/', geneProp,'_', addSignalPara,'.rds'))){
  ...
  ## add signal
  finishf <- list.files(paste0('./addsignal/data/plain/1256/','EM_FP_gene/', addSignalType,'/'))
  finishf <- finishf[!grepl('fdr', finishf)][1]
  fpdf <- readRDS(paste0('./addsignal/data/plain/1256/','EM_FP_gene/', addSignalType,'/', finishf))
  fp <- fpdf[seq(1, round(nrow(fpdf)*1)),1]
  set.seed(12345)
  selgene <- c(sample(row.names(expr), round(geneProp * nrow(expr)) - length(fp)), fp)
