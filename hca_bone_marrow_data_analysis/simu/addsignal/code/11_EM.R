...
if (!file.exists(paste0('./addsignal/result/', method, '/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
  res <- readRDS(paste0('./addsignal/data/HVGAlongGroup/1256/',addSignalType,'/', geneProp,'_', addSignalPara,'.rds'))
  expr <- res$expr
  selgene <- res$selgene
  ...
  if (method == 'permu'){
    if (!file.exists(paste0('./addsignal/result/permu/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
  ...
  if (method == 'permu_IR'){ ## ignore repetitive
    if (!file.exists(paste0('./addsignal/result/permu_IR/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
  ...
  if (method == 'EM'){
    print(method)
    dir.create(paste0('./addsignal/result/EM/',addSignalType,'/'), showWarnings = FALSE, recursive = TRUE)
    ...
    saveRDS(fdr, paste0('./addsignal/result/', method,'/',addSignalType,'/', geneProp,'_', addSignalPara,'_fdr.rds'))  
