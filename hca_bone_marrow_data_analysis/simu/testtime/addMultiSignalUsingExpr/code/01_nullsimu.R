...
# ----
if (method == 'EM_pm'){
testres <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=8, test.type = 'Time')
  saveRDS(testres, paste0(rdir, method, '/testres.rds'))
}
if (method == 'EM_chisq'){
testres <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=8, test.method='chisq', test.type = 'Time')
  saveRDS(testres, paste0(rdir, method, '/testres.rds'))
}
