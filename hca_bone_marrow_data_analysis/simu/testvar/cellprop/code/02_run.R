...
for (path in c('lymph', 'erythroid', 'monocyte')){
  ...
  rdir <- 
paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/hca/simu/testvar/cellprop/result/', path, 
'/')
  dir.create(rdir, recursive = T)
  for (prop in c(0,0.01,0.05, 0.25,seq(0.1, 0.9, 0.1))) {
    print(paste0(path, '_', prop))
    pseudotime = 
readRDS(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/hca/simu/testvar/cellprop/data/',path, 
'/', prop, '.rds'))
