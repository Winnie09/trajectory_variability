...
fn <- paste0(rdir, method,'/', signal, '1000.rds')
print(fn)
...
if (method == 'EM_NOT_centered'){
  ...
  names(pt) <- pseudotime[, 1]
  ### run test
  testres <- testpt(expr=expr, cellanno=cellanno, pseudotime=pt, design=design,ncores=1, permuiter=100, type = 'Variable', demean = FALSE)
  saveRDS(testres, fn)  
}
