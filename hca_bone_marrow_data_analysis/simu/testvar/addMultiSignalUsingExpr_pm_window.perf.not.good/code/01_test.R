...
pseudotime <- readRDS('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/hca/simu/testvar/nullsimu_addSampleVariation/data/data/pseudotime_pm.rds')
...
if (method == 'EM_pm'){
  ...
  ### run test
  system.time({
    testres <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=16, test.type = 'Variable', demean = FALSE, overall.only = F, test.method = 'permutation')
  })
  saveRDS(testres, fn)  
}
...
if (method == 'monocle2'){
  ...
  design = cbind(1,design)
  branch = sapply(1:nrow(cellanno), function(i) ((design[cellanno[i, 2], 2] == 1) + 0))
  res = monocle2_group(expr, pseudotime, branch)
  saveRDS(res, fn)  
}
