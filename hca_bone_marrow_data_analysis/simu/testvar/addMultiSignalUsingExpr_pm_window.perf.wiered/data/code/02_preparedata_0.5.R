library(here)
setwd(here('hca','simu','testvar','addMultiSignalUsingExpr_pm_window','data', 'data'))
source(here('function','01_function.R'))
source('/home-4/whou10@jhu.edu/scratch/Wenpin/resource/myfunc/01_function.R')
dir.create('fromgene', recursive = TRUE, showWarnings = FALSE)
dir.create('selgene', recursive = TRUE, showWarnings = FALSE)
dir.create('null', recursive = TRUE, showWarnings = FALSE)
...
cellanno_pm <- readRDS(paste0(ddir,'cellanno_pm.rds'))
design <- readRDS(paste0(ddir,'design.rds'))
pt <- readRDS(paste0(ddir,'pseudotime_pm.rds'))
saverlog <- readRDS(paste0(ddir,'saverlog_pm.rds'))
cnt <- readRDS(paste0(ddir,'cnt_pm.rds'))
pt = data.frame(cell = names(pt), pseudotime = pt, stringsAsFactors = F)
...
dir.create('count/', showWarnings = F, recursive = T)
dir.create('saver/', showWarnings = F, recursive = T)
j = 1 # signal from 1 weakest to 4 strongest
print(j)
...
m_with_halfmean <- m_with_halfmean/4  ## from 1/2 to 1/4

resexpr <-  rbind(savercnt[selgene1, ] + savercnt[fromgene[1:length(selgene1)], ]/2, ## 1/2 
                  savercnt[selgene2,] + pmsavercnt[fromgene[(length(selgene1)+1) : (length(selgene1)+length(selgene2))],]/2, ## 1/2
                  savercnt[selgene3,] + savercnt[fromgene[((length(selgene1)+length(selgene2)+1):(length(selgene1)+length(selgene2)+length(selgene3)))], ]/2) ## 1/2
...
add1 <- rbind(savercnt2[selgene1, ] + pmsavercnt2[fromgene[1:length(selgene1)], ]/2, ##1/2
              savercnt2[selgene2, ],
...
saveRDS(mat, paste0('saver/', j/2,'.rds'))

### count
m_with_halfmean <- pmcnt2[fromgene[((length(selgene1)+length(selgene2)+1):(length(selgene1)+length(selgene2)+length(selgene3)))],]
m_with_halfmean <- m_with_halfmean/4 ## from 1/2 to 1/4

resexpr <-  rbind(cnt[selgene1, ] + cnt[fromgene[1:length(selgene1)], ]/2, ## 1/2 
                  cnt[selgene2,] + pmcnt[fromgene[(length(selgene1)+1) : (length(selgene1)+length(selgene2))],]/2, ## 1/2
                  cnt[selgene3,] + cnt[fromgene[((length(selgene1)+length(selgene2)+1):(length(selgene1)+length(selgene2)+length(selgene3)))], ]/2) ## 1/2
mat <- rbind(resexpr[selgene, ], cnt[setdiff(rownames(cnt),selgene),])

add1 <- rbind(cnt2[selgene1, ] + pmcnt2[fromgene[1:length(selgene1)], ]/2, ## 1/2
              cnt2[selgene2, ],
              cnt2[selgene3, ] + m_with_halfmean)
cnt2.new <- rbind(add1, cnt2[setdiff(rownames(cnt2), selgene), ])
mat <- cbind(mat, cnt2.new[rownames(mat), ])
saveRDS(mat, paste0('count/', j/2,'.rds'))
