...
df1.nc[3,4] = (as.numeric(df1.nc[3,3])+as.numeric(df1.nc[3,5]))/2*0.99
df1.nc[4,4] = (as.numeric(df1.nc[4,3])+as.numeric(df1.nc[4,5]))/2*0.99
...
df1 <- sapply(af, function(f){
 ...
 # res = res[!rownames(res) %in% selgene1, ]
  res = res[order(res[,1], -res[,2]), ]
...
#names(colv) <- c('Lamian', 'Lamian.chisq', setdiff(sort(unique(pd[,2])), c('Lamian', 'Lamian.chisq')))
