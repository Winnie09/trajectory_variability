...
clu = Res$cluster
DDGType = Res$DDGType[names(clu)]
id <- which(DDGType %in% c('bothSig','meanSig','trendSig'))
clu <- clu[id]
DDGType <- DDGType[id]

res <- data.frame(clu = clu,DDGType = DDGType[names(clu)])
...
o1 <- rownames(res1)[order(res1$clu)]
o2 <- rownames(res2)[order(res2$clu)]
o3 <- rownames(res3)[order(res3$clu)]
...
print(plotClusterMeanAndDiff(Res, cluster = clu))
dev.off()
