...
df1.nc <- sapply(af, function(f){
  ...
  res = data.frame(pval=r$statistics[,'pval.overall'],adj.P.Val = r$statistics[,'fdr.overall'], zscore = r$statistics[,'z.overall'], stringsAsFactors = F)
  res = res[order(res[,1], res[,2]), ]
  
  res = res[!rownames(res) %in% rmgene, ]
  res$adj.P.Val <- p.adjust(res$pval,method='fdr')
...
df1 <- sapply(af, function(f){
  ...
  if ('df.diff.overall' %in% colnames(r$statistics)){
    res = data.frame(pval=r$statistics[,'pval.chisq.overall'],fdr = r$statistics[,'fdr.chisq.overall'], stat = (r$ll3-r$ll1)/r$statistics[,'df.diff.overall'], stringsAsFactors = F)
    res = res[order(res[,1], -res[,2]), ]
  } else {
    res = data.frame(pval=r$statistics[,'pval.chisq.overall'],fdr = r$statistics[,'fdr.chisq.overall'], stat = (r$ll3-r$ll1)/r$statistics[,'pval.chisq.overall'], stringsAsFactors = F)
    res = res[order(res[,1], res[,2]), ]
  }
