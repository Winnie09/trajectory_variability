...
print(i)
  s <- d[i,]
fac <- 4
  s[x==1][1:(sum(x==1)/fac)] <- s[x==1][1:(sum(x==1)/fac)] + 1e-3
  s[x==1][(sum(x==1)/fac*(fac-1)):sum(x==1)] <- s[x==1][(sum(x==1)/fac*(fac-1)):sum(x==1)] - 1e-3
  full_model_fit <- VGAM::vglm(s~x+sm.ns(pt, df=3)*x, epsilon=1e-1, family='uninormal')
  reduced_model_fit <- VGAM::vglm(s~sm.ns(pt, df=3), epsilon=1e-1, family='uninormal')
...
saveRDS(res,file=paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/hca/simu/testvar/addMultiSignalUsingExpr/result/monocle2_trajtest.corr/',f))
