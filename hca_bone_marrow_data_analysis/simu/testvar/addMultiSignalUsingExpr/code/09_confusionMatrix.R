...
for (i in 4:1){
  ...
  pdf(paste0(pdir, '/confusionMatrix_hm_colsum1.pdf'), width = 3, height = 2.5)
  print(pheatmap(tb1, cluster_rows = F, cluster_cols = F))
  dev.off()

  tb2 <- tb/rowSums(tb)
  pdf(paste0(pdir, '/confusionMatrix_hm_rowsum1.pdf'), width = 3, height = 2.5)
  print(pheatmap(tb2, cluster_rows = F, cluster_cols = F))
  dev.off()
}
