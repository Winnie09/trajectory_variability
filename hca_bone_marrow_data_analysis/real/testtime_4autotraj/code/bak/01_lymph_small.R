...
system.time({
  res <- testpt(expr=expr[1:100,], cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=16, test.type = 'Time', demean = FALSE, test.method = ifelse(m == 'EM_pm', 'permutation', 'chisq'), ncores.fit = 48)
})

saveRDS(res, paste0(rdir, 'testtime_res100.rds'))
