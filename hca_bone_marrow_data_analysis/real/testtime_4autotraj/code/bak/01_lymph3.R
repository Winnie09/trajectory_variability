...
gene <- c("LMAN2L:ENSG00000114988", "PRDX3:ENSG00000165672", "DPM1:ENSG00000000419")
system.time({
  res <- testpt(expr=expr[gene,], cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=16, test.type = 'Time', demean = FALSE, test.method = ifelse(m == 'EM_pm', 'permutation', 'chisq'), ncores.fit = 24)
})

saveRDS(res, paste0(rdir, 'testtime_res3.rds'))
