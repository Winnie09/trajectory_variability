...
library(here)
setwd(here())
ddir <- 'hca/real/build_from_tree_variability/result/clp/'
rdir <- paste0('hca/real/testvar/result/', m, '/clp/')
...
system.time({
  res <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design,ncores=24, test.type = 'Variable', demean = FALSE, test.method = ifelse(m == 'EM_pm', 'permutation', 'chisq'))
})
