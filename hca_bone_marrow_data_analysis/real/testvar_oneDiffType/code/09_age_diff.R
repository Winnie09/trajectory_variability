...
for (path in c('monocyte', 'lymph', 'erythroid')){
  ...
  res.lm <- readRDS(paste0('hca/real/testvar/result/', path, '/meandiff_age_res.rds'))
  ...
  saveRDS(mean(v2), paste0(rdir, path, '/meandiff_age_overlap.rds'))
  ...
  saveRDS(mean(v1), paste0(rdir, path, '/age_overlap.rds'))
  
  v1_pm <- sapply(seq(1,1e4), function(myseed){
    set.seed(myseed)
    #w1 = sample(targetgene, length(intersect(targetgene,u1)))
    #k1 <- cumsum(targetgene %in% w1)/seq(1:length(targetgene))
  ...
  library(ggplot2)
  pdf(paste0(pdir, path, '/agediff_hist_mean_overlap_proportion.pdf'), width = 3.5, height = 3)
  print(ggplot(data = pd, aes(x=agediff_pm)) + 
   ...
   geom_vline(xintercept = mean(v1), color = 'red') +
    ...
    ggtitle(paste0('age differential, p.value = ', pval)))
  dev.off()
  ...
  plotGenePopulation(Res, allg.full[1:min(length(allg.full), 25)], type = 'variable')
  dev.off()
