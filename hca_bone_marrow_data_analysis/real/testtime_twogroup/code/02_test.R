...
library(here)
setwd(here())
ddir <- 'hca/real/testtime_twogroup/data/data/'
...
pseudotime = readRDS(paste0(ddir, 'pt2.rds'))
...
ptcutid <- as.numeric(commandArgs(trailingOnly = T)[[2]])
ptqt <- quantile(fullpseudotime,seq(0,1,length.out = 6))
pseudotime <- pseudotime[pseudotime <= ptqt[ptcutid+1] & pseudotime > ptqt[ptcutid]]
...
if (method == 'EM_pm'){
  system.time({
    res <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=24, test.type = 'Time', demean = FALSE, test.method = 'permutation', ncores.fit = 24, permuiter=500)
  })
}
...
saveRDS(res, paste0(rdir, ptcutid, '_testtime_res.rds'))
