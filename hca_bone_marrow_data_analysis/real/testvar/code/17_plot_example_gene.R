...
for (path in c('erythroid', 'lymph', 'monocyte')){
 ...
 #             "trendSig"                "nonDDG"                "nonDDG" 
    ...
    gene = c("JPX:ENSG00000225470", rownames(res)[grepl('RCHY1', rownames(res))], 'EIF1AX:ENSG00000173674', "EIF1AY:ENSG00000198692", rownames(res)[grepl('UQCRC1', rownames(res))],  "PXN-AS1:ENSG00000255857")
    DDGType[gene]
    
    e = Res$expr
    for (g in gene){
      e[g, e[g,] > quantile(e[g, ], 0.99)] = NA
      e[g, e[g,] < quantile(e[g, ], 0.01)] = NA
    }
    Res$expr = e
 ...
 # NKAPP1:ENSG00000233382  UQCRC1:ENSG00000010256 PXN-AS1:ENSG00000255857 
 #             "trendSig"                "nonDDG"                "nonDDG" 
    pdf(paste0(pdir, path, '/gender/gene_example_all_types.pdf'), width = 7.5, height = 1.6)
    plotGene(Res, gene = gene, variable = 'gender', plot.point = T, sep = ':.*', continuous = F, point.alpha = 0.2, point.size = 0.2, line.size = 0.3, palette = 'Dark2', ncol = 6)
    dev.off()
  } 
}
