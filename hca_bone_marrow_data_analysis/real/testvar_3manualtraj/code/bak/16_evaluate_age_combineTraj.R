library(here)
# setwd('/Users/wenpinhou/Dropbox/trajectory_variability')
setwd(here())
ddir <- 'hca/real/testvar/result/EM_pm/'
rdir <-  'hca/real/testvar/result/EM_pm/combineTraj/'
pdir <- 'hca/real/testvar/plot/EM_pm/combineTraj/'
dir.create(pdir, recursive = T, showWarnings = F)
dir.create(rdir, recursive = T, showWarnings = F)
u1 <- readRDS('agediff/result/age_diffgene.rds')
source('function/01_function.R')
res.gtex <- readRDS('agediff/result/res.rds')

## ------------------------------------------------
## combine the gene orders from all trajectories ##
## ------------------------------------------------
reslist <- reslmlist <- list()
for (path in c('monocyte', 'lymph', 'erythroid')){
  print(path)
  res <- readRDS(paste0(ddir, path, '/age/age_fdr_res.rds'))
  res <- res[,1:9]
  res <- matrix(as.numeric(res), ncol=9, dimnames = dimnames(res))
  res <- res[order(res[,1], -res[,3]), ]
  rn <- sub(':.*', '', rownames(res))
  res <- res[!duplicated(rn), ]
  rownames(res) <- rn[!duplicated(rn)]
  res <- res[rownames(res) %in% rownames(res.gtex), ]
  reslist[[path]] <- res
  
  res.lm <- readRDS(paste0('hca/real/testvar/result/limma/', path, '/age_res.rds'))
  res.lm <- res.lm[order(res.lm[,5], -abs(res.lm[,1])), ]
  rn <- sub(':.*', '', rownames(res.lm))
  res.lm <- res.lm[!duplicated(rn), ]
  rownames(res.lm) <- rn[!duplicated(rn)]
  res.lm <- res.lm[rownames(res.lm) %in% rownames(res.gtex), ]
  reslmlist[[path]] = res.lm
}  

res = do.call(rbind, reslist)
reslm = do.call(rbind, reslmlist)

res <- res[order(res[,1], -res[,3]), ]
reslm <- reslm[order(reslm[,5], -abs(reslm[,1])), ]

## -------------
## Evaluation ##
## -------------
## our method overlap
targetgene <- rownames(res)
v1 <- cumsum(targetgene %in% intersect(targetgene,u1))/seq(1:length(targetgene))
v1 <- v1[seq(1,length(v1)) %% 10 == 0]
v1 <- mean(v1)
saveRDS(v1, paste0(rdir, '/age_overlap.rds'))

v1_pm <- sapply(seq(1,1e4), function(myseed){
  set.seed(myseed)
  sampgene <- sample(targetgene)
  k1 <- cumsum(sampgene %in% intersect(sampgene,u1))/seq(1:length(sampgene))
  k1 <- k1[seq(1,length(k1)) %% 10 == 0]
  mean(k1)
})
names(v1_pm) <- paste0('top',seq(1,length(v1_pm)))
pval <- mean(v1_pm > v1)

pd <- data.frame(agediff_pm = v1_pm, stringsAsFactors = FALSE)
library(ggplot2)
pdf(paste0(pdir, '/age_hist_overlap_proportion.pdf'), width = 2.8, height = 2.5)
print(ggplot(data = pd, aes(x=agediff_pm)) + 
        geom_histogram(aes(y=..density..), colour="black", fill="white")+
        geom_density(alpha=.2, fill="#FF6666") +
        geom_vline(xintercept = mean(v1), color = 'red') +
        theme_classic()+
        xlab('overlap proportion mean')+
        ggtitle(paste0('age, p.value = ', round(pval,3))))
dev.off()

## limmma overlap
targetgene2 <- rownames(reslm)
v2 <- cumsum(targetgene2 %in% intersect(targetgene2,u1))/seq(1:length(targetgene2))
v2 <- v2[seq(1,length(v2)) %% 10 == 0]
saveRDS(mean(v2), paste0(rdir,'/age_overlap_limma.rds'))
v2_pm <- sapply(seq(1,1e4), function(myseed){
  set.seed(myseed)
  sampgene <- sample(targetgene2)
  k1 <- cumsum(sampgene %in% intersect(sampgene,u1))/seq(1:length(sampgene))
  k1 <- k1[seq(1,length(k1)) %% 10 == 0]
  mean(k1)
})

names(v2_pm) <- paste0('top',seq(1,length(v2_pm)))
pval <- mean(v2_pm > mean(v2))
print(pval)

### limma 
pd <- data.frame(agediff_pm = v2_pm, stringsAsFactors = FALSE)
library(ggplot2)
pdf(paste0(pdir, '/age_hist_overlap_proportion_limma.pdf'), width = 2.8, height = 2.5)
print(ggplot(data = pd, aes(x=agediff_pm)) + 
        geom_histogram(aes(y=..density..), colour="black", fill="white")+
        geom_density(alpha=.2, fill="#FF6666") +
        geom_vline(xintercept = mean(v2), color = 'red') +
        theme_classic()+
        xlab('overlap proportion mean')+
        ggtitle(paste0('age, p.value = ', round(pval,3))))
dev.off()





