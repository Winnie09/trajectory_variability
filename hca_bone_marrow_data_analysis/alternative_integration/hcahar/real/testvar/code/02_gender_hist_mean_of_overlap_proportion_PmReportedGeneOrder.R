...
ddir <- rdir <- 'hcahar/real/testvar/result/'
pdir <- 'hcahar/real/testvar/plot/'
...
for (path in c('erythroid', 'lymph', 'monocyte')){
  ## read data
  Res <- readRDS(paste0(ddir, path, '/EM_pm.rds'))
  res <- Res$statistics
  ## identify DDGType 
  ...
  pdf(paste0(pdir, path, '/gender_hist_mean_of_overlap_proportion_PmReportedGeneOrder.pdf'), width = 5, height = 2.1)
