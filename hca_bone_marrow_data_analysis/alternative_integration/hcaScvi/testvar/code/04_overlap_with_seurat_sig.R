...
for (j in 1:3){
  ...
  seu.res = readRDS(paste0('hca/real/testvar/plot/EM_pm/', df[j,2], '/gender/numeric_res_with_clu.rds'))
  seu.s = seu.res[[1]]
...
saveRDS(ov, 'hcaScvi/testvar/compare/overlap_with_seurat_sig_res.rds')
saveRDS(ov.pm, 'hcaScvi/testvar/compare/overlap_with_seurat_sig_res_pm.rds')
saveRDS(fdrmat, 'hcaScvi/testvar/compare/fdrmat_seurat_sig_and_harmony.rds')
...
theme_set(.new_theme)
pdf('hcaScvi/testvar/plot/overlap_with_seurat_sig.pdf', height = 1.5, width = 2.4)
