...
gl <- matrix(NA,nrow=100,ncol=4,dimnames = list(NULL,c('HSC','Mono','Ery','Lym')))
#HSC
des <- cbind(1,grepl('HSC',colnames(alldata)))
res <- topTable(eBayes(lmFit(alldata,des)),coef=2,n=nrow(alldata))
res <- res[res[,1] > 0,]
gl[,'HSC'] <- rownames(res)[1:100]

des <- cbind(1,grepl('Mono',colnames(alldata)))
res <- topTable(eBayes(lmFit(alldata,des)),coef=2,n=nrow(alldata))
res <- res[res[,1] > 0,]
gl[,'Mono'] <- rownames(res)[1:100]

des <- cbind(1,grepl('Ery',colnames(alldata)))
res <- topTable(eBayes(lmFit(alldata,des)),coef=2,n=nrow(alldata))
res <- res[res[,1] > 0,]
gl[,'Ery'] <- rownames(res)[1:100]

des <- cbind(1,!grepl('HSC|Mono|Ery',colnames(alldata)))
res <- topTable(eBayes(lmFit(alldata,des)),coef=2,n=nrow(alldata))
res <- res[res[,1] > 0,]
gl[,'Lym'] <- rownames(res)[1:100]
