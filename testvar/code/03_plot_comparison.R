...
df <- lapply(af, function(f){
  ...
  r = readRDS(paste0(ddir, m, '/', f))
  res = r[['earlyDETest']]
  a = c(sub('.rds','',f), 'tradeSeq_earlyDETest', res[['sensfdr']][2:3])
  res = r[['patternTest']]
  b = c(sub('.rds','',f), 'tradeSeq_patternTest', res[['sensfdr']][2:3])
  res = r[['diffEndTest']]
  c = c(sub('.rds','',f), 'tradeSeq_diffEndTest', res[['sensfdr']][2:3])
  rbind(a, b, c)
})
