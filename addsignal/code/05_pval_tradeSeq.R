...
# rrr <- sapply(c(0.05, 0.1, 0.2), function(geneProp){
#   set.seed(12345)
#   selgene <- sample(row.names(expr), round(geneProp * nrow(expr)))
#   saveRDS(selgene, paste0('./addsignal/result/selectgene/', geneProp,'_genes.rds'))
# })
set.seed(12345)
selgene <- sample(row.names(expr), round(geneProp * nrow(expr)))
...
if (method == 'limma'){
...
} else if (grepl('tradeSeq', method)){
  dir.create(paste0('./addsignal/result/tradeSeq/',addSignalType,'/'), showWarnings = FALSE, recursive = TRUE)
  expr2 <- AddSignal(expr = expr, sample = sample, SelectGene = selgene, SelectSample = paste0('BM',c(1,2,5,6)), pseudotime = pseudotime, method = addSignalType, parameter=addSignalPara)
  counts <- round(exp(expr2 + 1))
  
  psn <- seq(1, length(pseudotime))
  names(psn) <- pseudotime
  pseudotime <- data.frame(curve1 = psn, curve2 = psn)
  rownames(pseudotime) <- names(psn)
  pseudotime = pseudotime[colnames(counts), ]
  
  v <- (allp %in% paste0('BM',c(1,2,5,6)) + 0)
  ...
  saveRDS(sce, paste0('./addsignal/result/tradeSeq/',addSignalType,'/', geneProp,'_', addSignalPara, '_sce.rds'))
 
  if (grepl('diffEndTest', method)){
    Res <- diffEndTest(sce)  
  } else if (grepl('patternTest', method)){
    Res <- patternTest(sce)  
  }
  ...
  saveRDS(final, paste0('./addsignal/result/tradeSeq/',addSignalType,'/', geneProp,'_', addSignalPara, '.rds'))
}
