...
if (!file.exists(paste0('./addsignal/data/HVGAlongGroup/1256/',addSignalType,'/', geneProp,'_', addSignalPara,'.rds'))){
  ...
  ## HVG in at least one group along pseudotime
  ...
  expr <- expr[intersect(rtgene[[1]], rtgene[[2]]), ] 
  ...
  ## save
  dir.create(paste0('./addsignal/data/HVGAlongGroup/1256/',addSignalType), recursive = TRUE)
  saveRDS(list(expr = expr2, selgene = selgene, pseudotime = pseudotime, design = design), paste0('./addsignal/data/HVGAlongGroup/1256/',addSignalType,'/', geneProp,'_', addSignalPara,'.rds'))  
}
