...
if (!file.exists(paste0('./addsignal/result/pmCellAnno/', method, '/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
  library(parallel)
  library(splines)
  library(limma)
  ...
  library(slingshot)
  dir.create(paste0('./addsignal/result/pmCellAnno/', method, '/', addSignalType,'/'), showWarnings = FALSE, recursive = TRUE)
  res <- readRDS(paste0('./addsignal/data/pmCellAnno/1256/',addSignalType,'/', geneProp,'_', addSignalPara,'.rds')) #####
  expr <- res$expr
  selgene <- res$selgene
  ...
  if (method == 'limma'){
    df <- t(sapply(0:20, function(i){
      ...
      saveRDS(r, paste0('./addsignal/result/pmCellAnno/limma/',addSignalType,'/',geneProp,'_',i,'.rds'))
      r <- r[order(r[,3]),]
    ...
    saveRDS(df, paste0('./addsignal/result/pmCellAnno/limma/',addSignalType,'/',geneProp,'_Para_FdrDiff_Area_Group1256.rds'))
  } 
  
  if (method == 'permu'){
    if (!file.exists(paste0('./addsignal/result/pmCellAnno/permu/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
      dr <- readRDS('./addsignal/result/dr.rds')
      ...
      names(v)[1] = 'Parameter'
      res[['sensfdr']] <- v
      saveRDS(res, paste0('./addsignal/result/pmCellAnno/permu/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))
    }
  } 
  
  if (method == 'permu_IR'){ ## ignore repetitive
    if (!file.exists(paste0('./addsignal/result/pmCellAnno/permu_IR/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
      dr <- readRDS('./addsignal/result/dr.rds')
      ...
      names(v)[1] = 'Parameter'
      res[['sensfdr']] <- v
      saveRDS(res, paste0('./addsignal/result/pmCellAnno/permu_IR/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))
    }
  } 
  
  if (method == 'permu_IRGroup'){ ## ignore repetitive
    if (!file.exists(paste0('./addsignal/result/pmCellAnno/permu_IRGroup/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))){
      dr <- readRDS('./addsignal/result/dr.rds')
      res <- exprdiff(expr=expr,design=design,sample=sample,dr=dr,pseudotime=pseudotime,permutation=TRUE, permutime=10000, IgnoreRepetitive = TRUE)
      r <- res[['res']]
      r <- r[order(r[,3]),]
      ...
      names(v)[1] = 'Parameter'
      res[['sensfdr']] <- v
      saveRDS(res, paste0('./addsignal/result/pmCellAnno/permu_IRGroup/', addSignalType,'/', geneProp,'_',addSignalPara,'.rds'))
    }
  } 
  ...
  if (method == 'EM'){
    ...
    saveRDS(fdr, paste0('./addsignal/result/pmCellAnno/', method,'/',addSignalType,'/', geneProp,'_', addSignalPara,'_fdr.rds'))  
