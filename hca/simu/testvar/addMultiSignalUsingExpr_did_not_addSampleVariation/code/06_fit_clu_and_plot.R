...
for (dataType  in seq(1,4)){
  ...
  rdir <- paste0('hca/simu/testvar/addMultiSignalUsingExpr/result/', method, '/',dataType)
  pdir <- paste0('hca/simu/testvar/addMultiSignalUsingExpr/plot/', method, '/',dataType)
  dir.create(rdir, recursive = TRUE, showWarnings = FALSE)
  dir.create(pdir, recursive = TRUE, showWarnings = FALSE)
  Res <- readRDS(paste0(rdir, '/Res_with_clu.rds'))
  ## -----------------------
  ## plotClusterMeanAndDiff
  ## -----------------------
  pdf(paste0(pdir, '/cluster_mean_and_diff.pdf'), width = 3.2, height = 6)
  ...
  # diffType <- getDDGType(Res, test.type = 'variable', test.method = ifelse(method == 'EM_pm', 'permutation', 'chisq'))
  # 
