...
pd[,5] = factor(as.character(pd[,5]), levels = c("overall","FDR.trendSig", "FDR.meanSig", "trend only","trend & mean", "mean only"))
pdf(paste0(pdir, 'compare_fdr_diff_auc_trenddiff_meandiff_2row.pdf'),width=6.5,height=4.8)
p1 <- ggplot(pd, aes(x = SignalStrength, y = Fdr.Diff, color=Method)) + 
  geom_point(size=2)  + 
  geom_line(size=1) + 
  theme_bw() + 
