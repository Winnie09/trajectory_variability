...
for (dataType in seq(1, 4)) {
  ...
  names(Res)
  res <- Res$statistics
  write.csv(res, paste0(rdir, '/Res_statistics.csv'))  
  ...
  col.diffType = colorRampPalette(brewer.pal(8, 'Set1'))(length(unique(diffType)))
  names(col.diffType) <- unique(diffType)
  col.signalType <-
