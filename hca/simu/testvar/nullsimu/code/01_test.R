...
method <- as.character(commandArgs(trailingOnly = FALSE)[1])
...
if (method == 'EM_NOT_centered'){
  res <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, ncores=2, permuiter=100, type = 'Variable', demean = FALSE)
  saveRDS(res, fn)
}
