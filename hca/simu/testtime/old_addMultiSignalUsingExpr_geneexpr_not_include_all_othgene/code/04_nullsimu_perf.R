...
Res = readRDS('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/hca/simu/testtime/old_without_demean/result/nullsimu/EM_SelectKnots/testres.rds')
res = data.frame(Res$fdr, Res$foldchange)
print(res[grep(g, rownames(res)),])
res = res[order(res[,1], -res[,2]), ]
res = res[res[,1]<0.05, ]
v <- nrow(res)
...
pdir <- '/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/hca/simu/testtime/plot/nullsimu/'
library(ggplot2)
pd = data.frame(method = names(num.fp), num.FP = num.fp, stringsAsFactors = F)
pd$method =factor(pd$method, levels = names(num.fp))
pdf(paste0(pdir, 'perf.pdf'),width=4,height=2)
