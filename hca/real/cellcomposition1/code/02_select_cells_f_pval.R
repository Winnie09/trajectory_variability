...
f_pval <- sapply(seq(1,1e3), function(myseed){
  ...
  c(f,sum(f_permute >= f)/num.permute)
})
rownames(f_pval) <- c('f','pvalue')
colnames(f_pval) <- paste0('seed_', seq(1,ncol(f_pval)))
saveRDS(f_pval, '/Users/wenpinhou/Dropbox/trajectory_variability/simu/data/setting/f_pval.rds')
...
pdf('/Users/wenpinhou/Dropbox/trajectory_variability/simu/plot/setting_dist_f_pval_fdr.pdf',width=7,height=7)
par(mfrow=c(2,2))
plot(f_pval[1,],f_pval[2,], main='', xlab='log10(f)', ylab='log10(p-value)', pch=20, cex=.5, log='xy')
plot(dist[seq(1,ncol(f_pval))],f_pval[2,], pch=20, main='', xlab='log10(dist)',ylab='log10(p-value)', cex=.5, log='xy')

plot(f_pval[1,],fdr, main='', xlab='log10(f)', ylab='log10(fdr)', pch=20, cex=.5, log='xy')
plot(dist[seq(1,ncol(f_pval))],fdr, pch=20, main='', xlab='log10(dist)',ylab='log10(fdr)', cex=.5, log='xy')
