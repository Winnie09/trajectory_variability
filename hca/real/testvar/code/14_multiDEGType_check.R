...
res <- res[res[,7] < 0.05, ]
DEGType <- as.character(res[,10])
names(DEGType) <- rownames(res)
tab1 <- table(DEGType[rownames(res)])

res.lm <- readRDS(paste0(ddir, path,'/meandiff_gender_res.rds'))
res.lm <- res.lm[res.lm[5] < 0.05, ]
tab2 <- table(DEGType[rownames(res.lm)])
tab3 <- table(DEGType[setdiff(rownames(res), rownames(res.lm))])
