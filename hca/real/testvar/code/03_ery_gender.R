...
system.time({
  res <- testpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design,ncores=16, test.type = 'Variable', test.method = ifelse(m == 'EM_pm', 'permutation', 'chisq'), cutoff =1e-3)
})
