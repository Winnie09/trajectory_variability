...
for (path in c('erythroid', 'lymph', 'monocyte')){
  ...
  ## plot DEG
  if (sum(res[,1] < 0.05) > 0){
    res <- cbind(res, DEGType = DEGType[rownames(res)], TrurPositive = ifelse(sub(':.*','', rownames(res)) %in% agediff, 'TRUE', 'FALSE'))
    saveRDS(res, paste0(rdir, '/age/age_fdr_res.rds'))
    write.csv(res, paste0(rdir, '/age/age_fdr_res.csv'))
    ## plot DEG in different types
    res <- Res$statistics
    res <- res[res[,1]<0.05, ,drop=F]
    res <- res[order(res[,'fdr.overall'], -res[,'z.overall']), ]
    DEGType <- DEGType[rownames(res)]
  ...
  ## identify DEGType
  DEGType <- getDEGType(Res)
  sink(paste0(rdir, '/gender/gender_DEGType.txt'))
  table(DEGType)
  sink()
  ## plot DEG
  if (sum(res[,1] < 0.05) > 0){
