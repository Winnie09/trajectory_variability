library(here)
setwd(here())
rdir <- './hca/real/build3traj/manual/result/lymph/'
pdir <- './hca/real/build3traj/manual/plot/lymph/'
...
table(ct)
id <- which(ct %in% c('HSC','MPP','LMPP', 'CLP', 'Bcell', 'CD8Tcell', 'CD4Tcell','NKcell')) ##
expr <- expr[,id]
...
pdf(paste0(pdir, 'pca.pdf'), width = 7, height = 3)
