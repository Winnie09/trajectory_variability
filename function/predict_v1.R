predict_fitting_v1 <- function(Knotnum, Design, Cellanno, Pseudotime, Resolution = 1000){
  ...
  ##
  philist.pred <- lapply(seq(0, max(Knotnum)), function(num.knot) {
    if (num.knot==0) {
      phi <- cbind(1,bs(seq(min(Pseudotime), max(Pseudotime), length.out = Resolution)))
    } else {
      knots = seq(min(Pseudotime),max(Pseudotime),length.out=num.knot+2)[2:(num.knot+1)]
      phi <- cbind(1,bs(seq(min(Pseudotime), max(Pseudotime), length.out = Resolution),knots = knots)) 

    }
  })
  names(philist.pred) <- as.character(0:max(Knotnum))
  ##
  ...
  pred <- do.call(rbind,lapply(unique(Knotnum),function(knot.num) {
    ...
    phi.pred <- philist.pred[[as.character(knot.num)]]
    do.call(cbind,lapply(names(sname),function(ss) {
      phiss <- phi[sname[[ss]],,drop=F]
      phiss.pred <- phi.pred[,,drop=F]
      # sexpr[[ss]] %*% (phiss %*% chol2inv(chol(crossprod(phiss)))) %*% t(phiss)
      tmp <- sexpr[[ss]] %*% (phiss %*% chol2inv(chol(crossprod(phiss)))) %*% t(phiss.pred)
      colnames(tmp) <- paste0(ss, ':', seq(1, Resolution))
      tmp
    }))
  }))
  pred <- pred[rownames(expr), ]
}
