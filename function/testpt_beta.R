testpt_beta <- function(expr, cellanno, pseudotime, design=NULL, permuiter=100, EMmaxiter=100, EMitercutoff=1, verbose=F, ncores=detectCores(), type='Time', test.pattern = 'overall', test.position = 'all', fit.resolution = 1000, return.all.data = TRUE, demean = FALSE) {
  set.seed(12345)
  library(splines)
  ...
  fit.model1 <- fitpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, EMmaxiter=EMmaxiter, EMitercutoff=EMitercutoff, verbose=verbose, ncores=1, test.pattern = test.pattern, test.position = test.position, model = 1)  
  orill.model1 <- sapply(fit.model1$parameter,function(i) unname(i$ll),USE.NAMES = F)[row.names(expr)]
  
  fit.model2 <- fitpt(expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design, EMmaxiter=EMmaxiter, EMitercutoff=EMitercutoff, verbose=verbose, ncores=1, test.pattern = test.pattern, test.position = test.position, model = 2)  
  orill.model2 <- sapply(fit.model2$parameter,function(i) unname(i$ll),USE.NAMES = F)[row.names(expr)]
  
  
  pval.21 <- pchisq(q = 2*(orill.model2 - orill.model1), df = 2*1, lower.tail = FALSE)
  pval.32 <- sapply(1:length(orill), function(i) pchisq(q = 2*(orill[i] - orill.model2[i]), df = knotnum[i] + 3, lower.tail = FALSE))

  library(limma)
  diffType <- cbind(meandiff = (pval.21 < 0.05),
                    trenddiff = (pval.32 < 0.05),
                    meantrenddiff = (pval < 0.05))
  ...
  if (return.all.data){
    if (demean){
      return(list(fdr = fdr, foldchange = foldchange, pvalue = pval, max.abs.beta2 = max.abs.beta2, parameter=orifit$parameter, orill=orill, perll = perll, knotnum = knotnum,  pseudotime = pseudotime[colnames(expr)], predict.values = pred[,colnames(expr)], design = design, cellanno = cellanno, expr.demean = expr, expr.ori = expr.ori, diffType = diffType))
    } else {
      return(list(fdr = fdr, foldchange = foldchange, pvalue = pval, max.abs.beta2 = max.abs.beta2, parameter=orifit$parameter, orill=orill, perll = perll, knotnum = knotnum,  pseudotime = pseudotime[colnames(expr)], predict.values = pred[,colnames(expr)], design = design, cellanno = cellanno, expr.ori = expr, diffType = diffType))
    } 
  } else {
    return(list(fdr = fdr, foldchange = foldchange, pvalue = pval, max.abs.beta2 = max.abs.beta2, parameter=orifit$parameter, orill=orill, perll = perll, knotnum = knotnum, predict.values = pred[,colnames(expr)], diffType = diffType))
  } 
}
