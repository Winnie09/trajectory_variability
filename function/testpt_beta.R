testpt_beta <- function(expr, cellanno, pseudotime, design=NULL, permuiter=100, EMmaxiter=100, EMitercutoff=1, verbose=F, ncores=detectCores(),type='Time', test.type = 'slope', test.position = 'all', fit.resolution = 1000) {
  ...
  pred <- predict(Knotnum = knotnum, Design = design, Cellanno = cellanno, Pseudotime = pseudotime, Resolution = fit.resolution)
  return(list(fdr = fdr, orill=orill, perll = perll, knotnum = knotnum, foldchange = foldchange, parameter=orifit$parameter, pseudotime = pseudotime, predict.values = pred, design = design, cellanno = cellanno))
}
