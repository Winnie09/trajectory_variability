fitpt_beta <- function(expr, cellanno, pseudotime, design, ori.design = design, test.pattern = 'overall', test.position = 'all',  maxknotallowed=10, EMmaxiter=100, EMitercutoff=1, verbose=F, ncores=1, model = 3) {
  ...
  sfit <- function(num.knot) {
    ...
    while (iter < EMmaxiter && length(gidr) > 0) {
      ...
      tmp <- lapply(names(xs), function(ss) {
        ...
        sapply(E_u_con_theta[[ss]],as.vector) + apply(k_s,1,tcrossprod)
      })
