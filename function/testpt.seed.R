testpt.seed <- function(expr, cellanno, pseudotime, design=NULL, permuiter=100, EMmaxiter=100, EMitercutoff=1, verbose=F, ncores=detectCores(), type='Time', test.pattern = 'overall', test.position = 'all', fit.resolution = 1000, return.all.data = TRUE, demean = FALSE) {
  set.seed(12345)
  library(splines)
  ...
  if (ncores == 1){
    ...
    fit <- mclapply(1:(permuiter+1),function(i){set.seed(i+100); fitfunc(i)}, mc.cores = ncores)
  }
  # << --- 20200926 
