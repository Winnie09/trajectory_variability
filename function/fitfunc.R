      ...
      print(paste0('iter ', iter, '\n'))
        if (test.type=='Time') {
          ...
          if (iter == 1){
             ...
             fitres.null <- fitpt(expr, cellanno, pseudotime, design, maxknotallowed=10, EMmaxiter=1000, EMitercutoff=0.1, verbose=F, ncores=1, model = mod.null, knotnum = fitres.full[[2]])
             if (exists('fitres.full') & exists('fitres.null')) {
                return(list(fitres.full = fitres.full, fitres.null = fitres.null))
              } else {
                return(NULL)
            ...
            fitres.null <- fitpt(perexpr, percellanno, psn, perdesign, maxknotallowed=10, EMmaxiter=1000, EMitercutoff=0.1, verbose=F, ncores=1, model = mod.null, knotnum = fitres.full[[2]])
           if (exists('fitres.full') & exists('fitres.null')) {
              return(list(fitres.full = fitres.full, fitres.null = fitres.null))
            } else {
              return(NULL)
