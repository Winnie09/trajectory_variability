...
fitpt <- function(expr, cellanno, pseudotime, design, knotnum=NULL, EMmaxiter=100, EMitercutoff=1, verbose=F, ncores=detectCores()) {
  ...
  sfit <- function(num.knot) {
    ...
    names(ll) <- gid
    while (iter < EMmaxiter && (iter == 0 || max(ll-llold) > EMitercutoff)) {
      llold <- ll
      E_phi_u_e <- phi_xs_beta <- phi_tau_phi <- list()
      M <- matrix(0,nrow=length(gid),ncol=nrow(design),dimnames=list(gid,names(sname)))
      for (ss in names(sname)) {
        ...
        E_phi_u_e[[ss]] <- sigma1s * (phi_xs_beta[[ss]]/phi_tau_phi[[ss]] + expr[gid,sname[[ss]]]/gamma[,ss]) - phi_xs_beta[[ss]]
        M[,ss] <- (rowSums((expr[gid,sname[[ss]],drop=F] - phi_xs_beta[[ss]] - E_phi_u_e[[ss]])^2) + rowSums(sigma1s))/ncol(sigma1s)
      }
      ...
      k_s <- lapply(names(sname),function(ss){
        tmp <- (expr[gid,sname[[ss]],drop=F] - phi_xs_beta[[ss]]) %*% phi[sname[[ss]],]
      ...
      tmp3 <- lapply(names(xs), function(ss){
        (expr[gid,sname[[ss]],drop=F] - E_phi_u_e[[ss]]) %*% phi_xs[[ss]] / gamma[,ss]
      })
      tmp3 <- Reduce('+', tmp3)     
      ...
      ll <- sapply(names(xs), function(ss) {
        ...
        rowSums(-log(pi * tmp)/2) - rowSums((expr[gid,sname[[ss]],drop=F] - phi_xs_beta[[ss]])^2 / tmp)
      })
      if (is.vector(ll)) {
        ll <- t(ll)
        row.names(ll) <- gid
