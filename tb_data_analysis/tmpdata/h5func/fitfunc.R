fitfunc <- function(iter, diffType = 'overall', gene = NULL, test.type = 'Time', EMmaxiter=100, EMitercutoff=0.05, verbose=F, ncores=1, expr=expr, cellanno=cellanno, pseudotime=pseudotime, design=design) {
  ...
  if (toupper(test.type)=='TIME') {
    if (iter == 1){
      ...
      ####
      fitres.full <- fitpt(expr=expr, pseudotime=perpsn, design=design, boot=boot,targetgene=gene,EMmaxiter=EMmaxiter, EMitercutoff=EMitercutoff, verbose=verbose, ncores=1, model = 1)
