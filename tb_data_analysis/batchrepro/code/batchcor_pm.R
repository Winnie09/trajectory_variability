...
des <- cbind(model.matrix(~sub('.*_','',samp2)),design[samp2,ncol(design)])
colnames(des)[ncol(des)] <- 'sex'
rownames(des) <- unname(samp2)

res <- testpt(paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/tb/data/sex/exprpc',pid,'.h5'), cellanno2, pt[cellanno2[,1]], design=des,testvar=ncol(des),test.type='Variable')

saveRDS(res,file=paste0('/home-4/whou10@jhu.edu/scratch/Wenpin/trajectory_variability/tb/batchrepro/res/batchcor/',seed,'.rds'))
